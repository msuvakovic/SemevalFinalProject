...
# from torch.utils.data import DataLoader, random_split
# from transformers import AutoTokenizer
# # Import model FIRST to ensure the xformers patch is applied before anything else checks dependencies
# from dataset_loader import ConspiracyDataset
# from features import PsychoLinguisticExtractor
# import argparse
# from sklearn.metrics import accuracy_score, classification_report
...
#     # 1. Setup Device
#     # Force usage of 'mps' (Metal Performance Shaders) on Mac if available, else CPU
...
#     # 2. Load NeoBERT Tokenizer
#     # Must use trust_remote_code=True to match the model's requirements
...
#         total_loss = 0
#         for batch in train_loader:
#             # Move all batch components to the active device
#             input_ids = batch['input_ids'].to(device)
#             mask = batch['attention_mask'].to(device)
#             feats = batch['manual_features'].to(device)
#             labels = batch['label'].to(device)
