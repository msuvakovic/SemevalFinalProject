...
try:
    ...
    BERTScorer = None  # pip install bert-score for BERT metric
...
def load_transcript(experiment, task):
    ...
    task_stem = task.split("_")[0] + ".TextGrid"
    grid_path = os.path.join(config.DATA_TEST_DIR, "test_stimulus", experiment, task_stem)
    if not os.path.exists(grid_path):
        grid_path = os.path.join(config.DATA_TEST_DIR, "TextGrids", task_stem)
    transcript_data = {}
...
class WER(object):
    ...
    def score(self, ref, pred):
        ...
        for ref_seg, pred_seg in zip(ref, pred):
            ref_str = " ".join(ref_seg) if isinstance(ref_seg, (list, tuple)) else ref_seg
            pred_str = " ".join(pred_seg) if isinstance(pred_seg, (list, tuple)) else pred_seg
            if len(ref_seg) == 0:
                error = 1.0
            else:
...
class BLEU(object):
    ...
    def score(self, ref, pred):
        ...
        for r, p in zip(ref, pred):
            r_str = " ".join(r) if isinstance(r, (list, tuple)) else r
            p_str = " ".join(p) if isinstance(p, (list, tuple)) else p
            if not r_str.strip() and not p_str.strip():
                results.append(1.0)
            elif not r_str.strip() or not p_str.strip():
                results.append(0.0)
            else:
...
class BERTSCORE(object):
    def __init__(self, idf_sents=None, rescale = True, score = "f"):
        if BERTScorer is None:
            raise ImportError("BERT metric requires bert_score. Install with: pip install bert-score")
    ...
    def score(self, ref, pred):
        ...
        # Handle empty strings to avoid BERTScore errors
        ref_strings = [s if s.strip() else "." for s in ref_strings]
        pred_strings = [s if s.strip() else "." for s in pred_strings]
