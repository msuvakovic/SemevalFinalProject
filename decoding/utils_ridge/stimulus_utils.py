...
def load_textgrids(stories, data_dir: str):
    ...
    for story in stories:
        # Check for both .TextGrid and .textgrid
        ...
        if not os.path.exists(grid_path):
            ...
            for f in os.listdir(base):
                if f.lower() == f"{story.lower()}.textgrid":
                    grid_path = os.path.join(base, f)
                    found = True
                    break
            if not found:
                print(f"Warning: TextGrid for {story} not found in {base}")
                continue
                
        try:
            grids[story] = TextGrid(open(grid_path).read())
        except Exception as e:
            print(f"Error loading TextGrid for {story}: {e}")
    return grids
